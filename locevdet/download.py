        ...
        for start_time, end_time in tqdm(events_times, desc=desc_msg, total=len(events_times)):
            ...
            if override or not os.path.isfile(mseed_save_path):
                ...
                try:
                    ...
                    all_seismograms_not_saved.append((station, start_time, end_time))
        if len(all_seismograms_not_saved)>0:
            print(f"{len(all_seismograms_not_saved)} seismograms haven't been saved : \
                {all_seismograms_not_saved}")
